...
TYPING_DELAY = config.get("typing_delay", 0.045)
DELETE_DELAY = config.get("delete_delay", 0.02)
...
# ── Listener lifecycle (used by menu bar app) ──────────────────────────────
...
def start_listener() -> None:
    ...
    with _listener_lock:
        ...
        _listener_instance = keyboard.Listener(on_press=on_press)
...
# ── Entry point (standalone terminal mode) ─────────────────────────────────
def main() -> None:
    W = 58
